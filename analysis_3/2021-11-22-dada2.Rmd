...
plotQualityProfile(fnFs[1:2]) +
  geom_vline(xintercept = c(17,250)) # trim & truncation length of fwd reads
```
```{r check rev reads of first two samples}
plotQualityProfile(fnRs[1:2]) +
  geom_vline(xintercept = c(17,250)) # trim & truncation length of rev reads
```
...
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, truncLen=c(200,200),
              ...
              compress=TRUE, multithread=TRUE, trimLeft=c(17,17)) # On Windows set multithread=FALSE
head(out)
```
...
head(track)
write.csv(track, here::here('output', 'dada2', 'track_nov22.csv'))
```
